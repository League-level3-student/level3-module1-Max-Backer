...
public class RetroSun extends PApplet {
	...
	ArrayList<Rectangle> sections = new ArrayList<>();
	float rectWidth = 2 * 150;
	...
	public void setup() {
		...
		for (int i = 0; i < 6; i++) {
			float startY = height / 2 + 150 + i * 50;

			sections.add(new Rectangle(width / 2 - 150, startY, rectWidth, rectHeight));
		}
	}
	...
	public void draw() {
		...
		for (int i = 0; i < pixels.length; i++) {
			...
			if (pixels[i] == sunColors[0]) {
				float step = map(y, HEIGHT / 2 - 150, HEIGHT + 150, 0, 2);
		...
		for (Rectangle r : sections) {
			r.y-=0.5;
			r.h = map(r.y, height / 2 - 50, height / 2 + 150, 1, 30);
